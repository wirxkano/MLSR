...
class IDN(nn.Module):
    def __init__(self, scale, num_features, d, s):
        ...
        self.fblock = FBlock(self.num_features)
        self.dblocks = nn.Sequential(*[DBlock(self.num_features, self.d, self.s) for _ in range(4)])
        self.deconv = nn.ConvTranspose2d(self.num_features, 3, kernel_size=17, stride=self.scale, padding=8, output_padding=1)
